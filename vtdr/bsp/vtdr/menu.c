...
unsigned char otdr2daydisplaynum = 1;
LCDTCB lcd_tcb;
LCDTCB last_lcd_tcb;
ACTION_TCB act_tcb;
...
void displayOverDriver()
{
	...
	//endtime = timechange(enddriverclk)-BCD2Char(enddriverclk.hour)*360-BCD2Char(enddriverclk.minute)*60;
	endtime = timechange(curTime)-BCD2Char(curTime.hour)*360-BCD2Char(curTime.minute)*60;
	startime = endtime-172800;
	do
	{
		if(STOPp > startbase)
		{
	...
	if(otdr2dayNum > 3)
	{
		for(j = 0;j<4;j++)
		{
			...
			if(numofdisplay>9)
			{
				...
				lcd_write_matrix(k,i*HAN_ZI,BCD2LCM(Char2BCD(numofdisplay),1),NUM);
				lcd_write_matrix(k,i*HAN_ZI+NUM,BCD2LCM(Char2BCD(numofdisplay),0),NUM);

			}
			else
			{
				lcd_write_matrix(k,i*HAN_ZI+NUM,BCD2LCM(numofdisplay,0),NUM);
			}
			numofdisplay ++;
		}
...
{
	unsigned char col=0,type=0;
	unsigned char buf=Parameter.typedata.type[0];
	lcd_clear(lineall);
	while((buf!='\0')&&(col<9))
	{
		if(buf<60){
		...
		col++;
		buf=Parameter.typedata.type[col];
	}
...
void DisplayStartTime()
{
	...
	lcd_write_matrix(line3,54,(FONT_MATRIX *)charater_twopoint,NUM);
	lcd_write_matrix(line3,60,BCD2LCM(temp_clock.minute,1),6);
	lcd_write_matrix(line3,66,BCD2LCM(temp_clock.minute,0),6);
	lcd_write_matrix(line3,72,(FONT_MATRIX *)charater_twopoint,NUM);
...
void DisplayEndTime()
{
	...
	unsigned long STOPp;
	CLOCK    temp_clock;
	STOPp = pTable.OverSpeedRecord.CurPoint-otdr2daydisplaynum*50+24;
...
void OKKeyHandler()
{
	...
	switch(lcd_tcb.mode)
	{
		case Node://
			...
			if((lcd_tcb.ListNb == 2) && (otdr2dayNum == 0))
			{
				//lcd_tcb.ListNb = -1;
				return;
			}
...
void DisplayNormalUI()
{
	...
	lcd_write_matrix(line1,0,(FONT_MATRIX *)charmapSinal,24);//radiotodn(radionum));
	lcd_write_matrix(line1,30,BCD2LCM(curTime.hour,1),NUM);
	lcd_write_matrix(line1,36,BCD2LCM(curTime.hour,0),NUM);
	lcd_write_matrix(line1,42,(FONT_MATRIX *)charater_twopoint1,NUM);
	lcd_write_matrix(line1,48,BCD2LCM(curTime.minute,1),NUM);
	...
	{
		lcd_write_matrix(line1,84,(FONT_MATRIX *)Letter_A,ZIMU);
	}
