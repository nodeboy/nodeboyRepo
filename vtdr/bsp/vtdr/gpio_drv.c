...
void rt_hw_EXTI_cfg()
{
	...
	EXTI_InitStructure.EXTI_Trigger = EXTI_Trigger_Falling; //设置中断触发方式为上沿触发
...
void rt_hw_gpio_init(void)
{
	...
	GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_IPU;
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_12 | GPIO_Pin_11 | GPIO_Pin_0;//速度，喇叭
    GPIO_Init(GPIOE, &GPIO_InitStructure);
...
void rt_iwtdog_init(void)
{
	...
	IWDG_SetReload(0xfff); //设置 IWDG 重装载值
