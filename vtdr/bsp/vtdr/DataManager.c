...
unsigned char  DriverFlag;//1:the same driver 0:not the same driver
...
int InitializeTable()
{
	...
	memcpy(Parameter.typedata.type,"WH08V3004",9);
...
void ValueStatusHandler()
{
	...
	if(Datastatusdata.keepstart10s)
	{
		if((DriverStatus & DRIVING_STAR)==0)
		{
			...
			memcpy((unsigned char *)&overdriverdata.startlocation,(unsigned char *)&location,strlen((unsigned char *)&location));
		...
		timedriverstartmin = timechange(startdriverclk)/60;
		timenowmin = timechange(curTime)/60;
	...
	else if(Datastatusdata.keepstop10s)
	{
		if((DriverStatus & DRIVING_STOP)==0)
		{
			...
			memcpy((unsigned char *)&overdriverdata.endlocation,(unsigned char *)&location,strlen((unsigned char *)&location));
		}
		DriverStatus |= DRIVING_STOP;
		//DriverStatus &= ~DRIVING_STAR;
		timedriverstartmin = timechange(enddriverclk)/60;
		timestopmin = timechange(curTime)/60;
		if((((timenowmin-timestopmin)>20)||(DriverFlag ==0))&&(DriverStatus & DRIVING_STAR))//并且同一个驾驶人
		{
			DriverFlag = 1;
			DriverStatus &= ~DRIVING_STAR;
...
{
	int i,pt;
	if((DriverStatus & DRIVING_STAR )&&((DriverStatus & DRIVING_STOP)!= DRIVING_STOP))
	{
...
void OverDriverHandler()
{
	...
	for(i = 0;i<8;i++ )
	{
		overdriverdata.startdrivertime.year =curTime.year;
		overdriverdata.startdrivertime.month =curTime.month;
		overdriverdata.startdrivertime.day =curTime.day;
		overdriverdata.startdrivertime.hour =curTime.hour+i-17;
		overdriverdata.startdrivertime.minute =curTime.minute;
		overdriverdata.startdrivertime.second =curTime.second;

		overdriverdata.enddrivertime.year =curTime.year;
		overdriverdata.enddrivertime.month =curTime.month;
		overdriverdata.enddrivertime.day =curTime.day;
		overdriverdata.enddrivertime.hour =curTime.hour+i-16;
		overdriverdata.enddrivertime.minute =curTime.minute;
		overdriverdata.enddrivertime.second =curTime.second;
		WriteRecordData2Flash(&pTable.OverSpeedRecord,(unsigned char *)&overdriverdata,50);//write the data
	}
#endif
}
