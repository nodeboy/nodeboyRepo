...
{
		{
			0xD6,0xB4,0xD0,0xD0,0xB1,0xEA,0xD7,0xBC,//执行标准版本年号
			0xB0,0xE6,0xB1,0xBE,0xC4,0xEA,0xBA,0xC5

		},
		{
			0xB5,0xB1,0xD8,0xDD,0xBC,0xDD,0xCA,0xBB,//当前驾驶人信息
			0xC8,0xCB,0xD0,0xC5,0xCF,0xA2
		},
		{
			0xB6,0xA8,0xCA,0xB1,0xCA,0xB1,0xBC,0xE4  //定时时间
		},
		{
			0xC0,0xDB,0xBC,0xC6,0xD0,0xD0,0xCA,0xBB,  //累计行驶里程
			0xC0,0xEF,0xB3,0xCC
		},
		{
			0xC2,0xF6,0xB3,0xE5,0xCF,0xB5,0xCA,0xFD //脉冲系数
		},
		{
			0xB3,0xB5,0xC1,0xBE,0xD0,0xC5,0xCF,0xA2  //车辆信息
		},
		{
			0xD7,0xB4,0xCC,0xAC,0xD0,0xC5,0xBA,0xC5,  //状态信号配置信息
			0xC5,0xE4,0xD6,0xC3,0xD0,0xC5,0xCF,0xA2
		},
		{
			0xBC,0xC7,0xC2,0xBC,0xD2,0xC7,0xCE,0xA8,  //记录仪唯一性编号
			0xD2,0xBB,0xD0,0xD4,0xB1,0xE0,0xBA,0xC5
		},
		{
			0xD0,0xD0,0xCA,0xBB,0xCB,0xD9,0xB6,0xC8,  //行驶速度记录
			0xBC,0xC7,0xC2,0xBC
		},
		{
			0xCE,0xBB,0xD6,0xC3,0xD0,0xC5,0xCF,0xA2 ,  //位置信息记录
			0xBC,0xC7,0xC2,0xBC
		},
		{
			0xCA,0xC2,0xB9,0xCA,0xD2,0xC9,0xB5,0xE3 ,  //事故疑点记录
			0xBC,0xC7,0xC2,0xBC
		},
		{
			0xB3,0xAC,0xCA,0xB1,0xBC,0xDD,0xCA,0xBB,  //超时驾驶记录
			0xBC,0xC7,0xC2,0xBC
		},
		{
			0xBC,0xDD,0xCA,0xBB,0xB3,0xAC,0xC9,0xED ,  //驾驶人身份记录
			0xB7,0xDD,0xBC,0xC7,0xC2,0xBC
		},
		{
			0xCD,0xE2,0xB2,0xBF,0xB7,0xA6,0xB5,0xE7 ,  //外部供电记录
			0xBC,0xC7,0xC2,0xBC

		},
		{
			0xB2,0xCE,0xCA,0xFD,0xD0,0xDE,0xB8,0xC4 ,  //参数修改记录
			0xBC,0xC7,0xC2,0xBC
		},
		{
			0xCB,0xD9,0xB6,0xC8,0xD7,0xB4,0xCC,0xAC,  //速度状态日志
			0xC8,0xD5,0xD6,0xBE
		},
...
#endif
	filenameString[0] = 0x30;
	filenameString[1] =0x3a;
	filenameString[2] = 0x44;
	filenameString[3] =BCD2ASCLL(curTime.year,1);
	filenameString[4] = BCD2ASCLL(curTime.year,0);
	filenameString[5] =BCD2ASCLL(curTime.month,1);
	filenameString[6] = BCD2ASCLL(curTime.month,0);
	filenameString[7] = BCD2ASCLL(curTime.day,1);
	filenameString[8] = BCD2ASCLL(curTime.day,0);
	filenameString[9] =0x5f;
	filenameString[10] = BCD2ASCLL(curTime.hour,1);
	filenameString[11] =BCD2ASCLL(curTime.hour,0);
	filenameString[12] = BCD2ASCLL(curTime.minute,1);
	filenameString[13] =BCD2ASCLL(curTime.minute,0);
	filenameString[14] = 0x5f;
	filenameString[15] = Parameter.AutoInfodata.AutoCode[0];
	filenameString[16] =Parameter.AutoInfodata.AutoCode[1];

	filenameString[17] = Parameter.AutoInfodata.AutoCode[2];
	filenameString[18] =Parameter.AutoInfodata.AutoCode[3];

	filenameString[19] = Parameter.AutoInfodata.AutoCode[4];
	filenameString[20] =Parameter.AutoInfodata.AutoCode[5];
	filenameString[21] = Parameter.AutoInfodata.AutoCode[6];
	filenameString[22] =Parameter.AutoInfodata.AutoCode[7];

	filenameString[23] = 0x2e;
	filenameString[24] =0x56;

	filenameString[25] = 0x44;
	filenameString[26] =0x52;
}
void CacuTheChecksum(uint8_t *writebuf,uint16_t cslenth)
{
	uint16_t i;
	for(i=0;i<cslenth;i++)
	{
		usbchecksum = usbchecksum^writebuf[i];
	}
...
void WriteTheData(unsigned short num)
{
		...
		uint8_t writeTextBuff[40]={
									0x34,0x56,0x78,0x9a,0x12,0x12,0x12,0xbc,0xde,0x12,
									0x34,0x56,0x78,0x9a,0x12,0x12,0x12,0xbc,0xde,0x12,
									0x34,0x56,0x78,0x9a,0x12,0x12,0x12,0xbc,0xde,0x12,
									0x34,0x56,0x78,0x9a,0x12,0x12,0x12,0xbc,0xde,0x12,
								};
		...
		uint16_t Wcount=0,i;
		uint32_t bytesWritten,bytesHaveWrite=0;;
		uint16_t bytesToWrite;
		FRESULT res;//modify by leiyq 20120319
		usbchecksum = 0;
		bytesToWrite = FilltheTextBuff(writeTextBuff,0,&Wcount);
		CacuTheChecksum(writeTextBuff,bytesToWrite);
		res= f_write (&file, writeTextBuff, bytesToWrite, (void *)&bytesWritten);
		if((bytesWritten == 0) || (res != FR_OK)) /*EOF or Error*/
		{
			rt_kprintf("> STM32.TXT CANNOT be writen.\n");
		}
		bytesHaveWrite = bytesToWrite+bytesHaveWrite;
		f_lseek(&file,bytesHaveWrite);
		for(i = 0x08;i<0x16;i++)
		{
		...
		res= f_write (&file, &usbchecksum, 1, (void *)&bytesWritten);
		if((bytesWritten == 0) || (res != FR_OK)) /*EOF or Error*/
		{
			rt_kprintf("> STM32.TXT CANNOT be writen.\n");
		}
...
unsigned short FillthedataBlock(unsigned char *buf,unsigned char block,unsigned short *count)
{
...
unsigned short FilltheTextBuff(unsigned char *bufptr,unsigned char block,unsigned short *Count)
{
